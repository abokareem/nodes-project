...
export default {
  ...
  created () {
    request.adminGetAllNodeWithdrawals(this.$i18n.locale).then(res => {
      ...
      for (let index in resWithdrawals) {
        this.withdrawals.data.push({
          ...
          nodeId: resWithdrawals[index].node_id,
          currency: resWithdrawals[index].currency.name,
  ...
  methods: {
    approve (withdrawal) {
      request.approveNodeWithdrawal(withdrawal.id, this.$i18n.locale).then(res => {
    ...
    decline (withdrawal) {
      request.declineWithdrawal(withdrawal.id, this.$i18n.locale).then(res => {
        response.handleSuccess(res, this)
